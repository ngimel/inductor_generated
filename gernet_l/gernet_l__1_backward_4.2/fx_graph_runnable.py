...
class Repro(torch.nn.Module):
    ...
    def forward(self, sub_1, unsqueeze, tangents_1):
        full = torch.ops.aten.full.default([], 8.0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        div_1 = torch.ops.aten.div.Tensor(tangents_1, 10.0);  tangents_1 = None
        div_2 = torch.ops.aten.div.Tensor(div_1, full);  div_1 = full = None
        full_like = torch.ops.aten.full_like.default(sub_1, 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False, memory_format = torch.preserve_format)
        scatter = torch.ops.aten.scatter.value(full_like, 1, unsqueeze, -1.0);  full_like = unsqueeze = None
        mul = torch.ops.aten.mul.Tensor(scatter, div_2);  scatter = div_2 = None
...
args = [((8, 1000), (1000, 1), torch.float32, 'cuda'), ((8, 1), (1, 1), torch.int64, 'cuda'), ((), (), torch.float32, 'cuda')]
